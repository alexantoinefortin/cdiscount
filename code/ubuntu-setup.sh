...
sudo apt-get -y install cmake git unzip pkg-config
...
bazel build -c opt --copt=-mavx --copt=-mavx2 --copt=-mfma --copt=-msse4.2 --copt=-mfpmath=both --config=cuda --cxxopt="-D_GLIBCXX_USE_CXX11_ABI=0" //tensorflow/tools/pip_package:build_pip_package
...
sudo pip3 install tensorflow-1.4.1-cp36-cp36m-linux_x86_64.whl
